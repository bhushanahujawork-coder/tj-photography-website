...
/**
 * Client-facing gallery entry — used by the platform client portal (/gallery)
 * and public share links (/s/[code]). Looking up a wedding by its access code
 * is done against the public backend endpoint `/api/v1/weddings/by-code/{code}`.
 */
export function CustomerGallery({ code }: { code?: string }) {
  const [searchCode, setSearchCode] = useState(code || '')
  const [searched, setSearched] = useState(!!code)
  const [wedding, setWedding] = useState<Wedding | null>(null)
  const [loading, setLoading] = useState(false)

const lookup = (queryCode: string) => {
  apiFetch<Wedding>(`/api/v1/weddings/by-code/${queryCode}`)
    .then(setWedding)
...
useEffect(() => {
  if (code) lookup(code.toUpperCase().trim())
}, [code])
